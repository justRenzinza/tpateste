...
public class ArvoreAVL<T> extends ArvoreBinaria<T> {
    ...
    private No<T> adicionarRecursivamente(No<T> noAtual, T novoValor) {
        if (noAtual == null) {
            return new No<>(novoValor); // serve para criar um nó novo pra caso o nó atual for null
        }
        ...
        if (comparacao < 0) {
            ...
            // só chega aq se o nó tiver o valor duplicado
    ...
    @Override
    public T remover(T valor) { // o bom de usar a forma recursiva aqui, é q já da pra balancear a arvore
    ...
    private No<T> encontrarMinimo(No<T> no) { // encontra o nó com o menor valor, por isso ele so vai pra esquerda
    ...
    private No<T> balancear(No<T> no) {
        int fator = fatorDeEquilibrio(no); // necessario pra balancear a arvore
        ...
        return no; // retorna o no depois de balancear
    }
